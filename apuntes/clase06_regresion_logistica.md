...
**La probabilidad de no pagar, por tramo de deuda** (slide 10). Se agrupan los clientes por
...
## Etapa 3: la regresión logística (slides 12 a 21)
...
**La sigmoide** (slide 18) es la función que apareció al despejar: toma el
...
**Cómo se lee b** (slide 19). Con la deuda en cientos de dólares, b = 0,55 y
...
**Cómo se ajusta** (slide 20). No por mínimos cuadrados, porque con un 0/1 no
...
con la misma sintaxis de fórmulas que `smf.ols` (slide 21), y `predict`
entrega probabilidades.

## Etapa 4: el mismo summary (slides 22 a 30)
...
**Varias variables** (slides 24 y 25). Solos, los estudiantes dejan de pagar
...
**El modelo final de los clientes, escrito** (slide 26):

    log(P / (1 − P)) = −10,87 + 0,57·deuda₁₀₀ − 0,65·estudiante + 0,003·ingreso

donde deuda₁₀₀ es la deuda en cientos de dólares, estudiante vale 1 o 0 e
ingreso está en miles de dólares; P sale de la sigmoide, P = 1 / (1 + e^(−z))
con z la recta. Un estudiante con 2.000 dólares de deuda y 20 mil de ingreso:
z = −10,87 + 0,57·20 − 0,65 + 0,003·20 = 0,02, y P = 0,50, justo en la
frontera.
...
**El Titanic** (slides 27 a 30). 891 pasajeros; el 74% de las mujeres
...
**El modelo del Titanic, escrito** (slide 29):
...
## Etapa 5: clasificar (slides 31 a 38)

**El umbral** (slide 32). Un clasificador convierte la probabilidad en un sí
o un no con un umbral; el más simple es 0,5. Con él, el modelo dice "no paga"
desde los 1.937 dólares de deuda.

**La matriz de confusión** (slide 33) cruza lo real (filas) con lo predicho
...
no el modelo (slide 34): para el banco, el falso negativo es dinero perdido y
...
**La exactitud** (slide 35) es la fracción de aciertos: la diagonal de la
...
**La exactitud engaña cuando el sí es raro** (slide 36). Comparadas: el modelo acierta el 97,3%, pero decir que nadie deja de
...
**El Titanic como clasificador** (slide 37). Con un sí frecuente (41%), la
...
**Evaluar fuera de la muestra** (slide 38). Medir la exactitud en los mismos
...
## La respuesta a la pregunta de la clase (slides 39 y 40)
